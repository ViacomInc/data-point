...
describe('ResolveEntity.resolveMiddleware', () => {
  test('It should execute a middleware', () => {
    dataPoint.middleware.use('source:before', (acc, next) => {
      acc.value = 'bar'
      next(null)
    })
  ...
  test('It should execute a middleware', () => {
    ...
    const racc = helpers.createAccumulator('foo')
    return ResolveEntity.resolveMiddleware(dataPoint, 'source:before', racc)
      .catch(reason => reason)
      .then(reason => {
...
describe('ResolveEntity.resolveEntity', () => {
  const resolver = (acc, resolveTransform) => Promise.resolve(acc)
