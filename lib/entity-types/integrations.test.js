...
test('Entry#resolve - resolve source', () => {
  const expected = {
    ok: true
  }
  ...
  return dataPoint.transform('entry:callSource', {}).then(result => {
    expect(result.value).toEqual(expected)
  })
})

test('Entry#resolve - source uses locals object', () => {
  const expected = {
    ok: true
  }
...
test('Entry#resolve - resolve hash with source', () => {
  const expected = {
    ok: true
  }
  ...
  return dataPoint.transform('entry:hashThatCallsSource').then(result => {
    expect(result.value).toEqual(expected)
  })
})

test('Entry#resolve - resolve hash with source and hash reducers', () => {
  const expected = {
    newOk: 'trueok',
    ok: true
  }
  ...
  return dataPoint
    .transform('entry:callHashWithSourceAndExtendResult')
    .then(result => {
      expect(result.value).toEqual(expected)
    })
})
...
test('Entry#resolve - resolve model with multiple sources', () => {
  ...
  return dataPoint
    .transform('entry:callHashThatCallsMultipleSources')
    .then(result => {
      expect(result.value).toEqual(expected)
    })
})
...
test('Entry#resolve:middleware(entry:after) - gets called', () => {
  ...
  dataPoint.middleware.use('entry:after', (acc, next) => {
    expect(acc.context.id).toBe('entry:callSource')
    next(null)
  })
